...
ringbuffer_t* create_ringbuffer_shm(const char* name, uint64_t tuple_size) {
    ...
    if (ret == NULL) {
        LOG_ERR("ringbuffer_t* malloc() failed: %d\n", errno);
    }

    // setup name, fd, tuple size
    ret->name = (char*)malloc(strlen(name) + 1);
    if (ret == NULL) {
        LOG_ERR("ringbuffer_t* name malloc() failed: %d\n", errno);
    }
                        ...
                        unsigned core_id, int numa_id)
{
    ...
    char name[30];
    if (core_id == NON_PMD_CORE_ID) {
        sprintf(name, "/rb_%d", 0);
    } else {
        sprintf(name, "/rb_%d", index + 1);
        index += 1;
    }
               ...
               bool md_is_valid, odp_port_t port_no)
{
    ...
    DP_PACKET_BATCH_REFILL_FOR_EACH (i, cnt, packet, packets_) {
        ...
        if (likely(mbuf != NULL)) {  
            ...
            //if (eth_hdr->ether_type == rte_cpu_to_be_16(ETHER_TYPE_IPv4) || eth_hdr->ether_type == rte_cpu_to_be_16(ETHER_TYPE_VLAN)) {
                struct ipv4_hdr *ip_hdr;
                tuple_t tuple;
                ip_hdr = rte_pktmbuf_mtod(mbuf, struct ipv4_hdr *);
                //tuple.key.src_ip = ip_hdr->src_addr;
                tuple.key.src_ip = *(uint32_t*)((unsigned char *)eth_hdr + 26);
                //tuple.key.dst_ip = ip_hdr->dst_addr;
                //tuple.key.src_port = ip_hdr->packet_id;
                tuple.key.src_port = *(uint16_t*)((unsigned char *)eth_hdr + 18);
                tuple.size = mbuf->pkt_len;
                //tuple.size = ntohs(*(uint16_t*)((unsigned char *)eth_hdr + 16));
                //if (tuple.key.src_port != 0x0)
